...
| 02 | Project Midnight Migration: Azure Storage Administration Challenge | [A scenario-driven training module for enterprise Azure Storage architecture, security, and cost optimization using the Azure CLI](./02-midnight-migration-ctf) | AZ-104 · AZ-500 |
...
**Zero-Trust Access Control**: Implement Entra ID B2B guest access and container-scoped RBAC, entirely eliminating the use of connection strings and shared access keys.
**Data Resilience**: Configure and validate Blob and Container Soft Delete mechanisms, including recovery procedures for virtual directories.
**Version Control & Auditing**: Establish an immutable paper trail using Blob Versioning and Snapshots to protect against accidental overwrites.
**Architectural Selection**: Evaluate the technical and authentication boundaries between Blob Storage and Azure Files for lift-and-shift legacy workloads.
**Cost Management**: Embedded financial context detailing the hidden billing impacts of access tiering, retention policies, and storage redundancy.
